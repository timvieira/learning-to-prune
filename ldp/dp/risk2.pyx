...
#                                cell.insert(X)
                            ...
                            self.B[I,K,X,0] = add(was, mul(mul(left, right), rule))
...
#                    cell.insert(X)
    ...
    cdef inline void unary_callback(self, int I, int K, int X, int Y) nogil:
        ...
        if self.grammar.fine2coarse_int[X] == self.gold_coarse[I,K]:
        ...
        if self.mode == INSIDE:
            self.B[I,K,X,1] = add(self.B[I,K,X,1], mul(self.B[I,K,Y,0], rule))

        elif self.mode == OUTSIDE:
            self.A[I,K,Y,0] = add(self.A[I,K,Y,0], mul(self.A[I,K,X,1], rule))
...
#            self.B[I,K,Y,None] = self.B[I,K,Y,None] + self.A[I,K,X,0] * rule
    ...
    cdef void outside(self) nogil:
        ...
        dense_cell = new intvec()
        for Y in range(self.NT):
            dense_cell.push_back(Y)
...
#                    for Y in deref(self._span_index[tri(I,J)]):
                    ...
                    if span_zero:
                        left_cell = self._span_index[tri(I,J)]
                    else:
                        left_cell = dense_cell

                    for Y in deref(left_cell):

#                    for Y in range(self.NT):
                        left = self.B[I,J,Y,1]
                        left_zero = is_zero(left)

                        # Note: You could use spandex for looping over Y when span is zero
                        if left_zero and span_zero:
                            continue
                        ...
                        for br in deref(self.grammar.r_y_xz[Y]):   # left-child loop
                            ...
                            right = self.B[J,K,Z,1]
                            right_zero = is_zero(right)
                            if right_zero and span_zero:
                                continue
                            ...
                            d_times_span = mul(d, span)

                            if not left_zero:
                                self.A[J,K,Z,1] = add(self.A[J,K,Z,1], mul(d_times_span, left))

                            if not right_zero:
                                self.A[I,J,Y,1] = add(self.A[I,J,Y,1], mul(d_times_span, right))


#                    for Z in deref(self._span_index[tri(J,K)]):
#                        right = mul(self.B[J,K,Z,1], span)
...
#
#                            was = self.A[I,J,Y,1]
#                            self.A[I,J,Y,1] = add(was, mul(d, right))
